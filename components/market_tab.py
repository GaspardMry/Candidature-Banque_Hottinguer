...
def render(df: pd.DataFrame, returns: pd.DataFrame):
    ...
    st.markdown('<div class="terminal-header">RÉGIME DU MARCHÉ DU RECRUTEMENT</div>', unsafe_allow_html=True)
    ...
    fig_market.update_yaxes(title="Indice de marché")
    fig_market.update_xaxes(title="Date")
    ...
    fig_capture.update_yaxes(title="Capture Ratio (%)")
    ...
    info_box(
        ...
        "de hausse qu'il ne subit de baisse, il s'agit de la <b>convexité positive</b>. "
        "La métrique <b>Neuro</b> affiche le ratio le plus élevé dans cette simulation : "
        "hypothèse retenue -> un profil neuroatypique s'adapte plus vite en période de crise "
        "qu'un profil conventionnel. "
        "<i>L'indice de marché utilisé est un proxy synthétique ; une validation sur données réelles "
        "de recrutement renforcerait cette analyse.</i>",
        style="orange",
    )
    ...
    for col_name in df.columns:
        ...
        with col1:
            ...
            fig_box.update_yaxes(tickformat=".0%", title="Probabilité")
