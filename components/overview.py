...
def render(df: pd.DataFrame):
    ...
    fig.update_yaxes(tickformat=".0%", title="Probabilité de succès")
    ...
    with col1:
        info_box(
            ...
            "événements réels (certifications obtenues, refus essuyés, pics d'énergie). "
            "L'ensemble forme un <b>portefeuille de risques personnels</b> analysable avec "
            "les outils classiques utilisés en Risk.",
            style="cyan",
    ...
    with col2:
        info_box(
            ...
            f"La métrique la plus volatile est <b>ma Neurodivergence</b> "
            f"(σ = {df['Probabilité_Neuro'].std():.1%})" 
             " avec des pics d'hyperfocus que je ne maîtrise pas encore à la demande. "
    ...
    fig_ret.update_yaxes(tickformat=".0%", title="Rendement quotidien")
    fig_ret.update_xaxes(title="Date")
