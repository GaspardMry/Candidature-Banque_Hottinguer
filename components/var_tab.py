...
def render(df: pd.DataFrame, returns: pd.DataFrame):
    ...
    st.markdown(
        '<div class="terminal-header">VALUE AT RISK',
    ...
    info_box(
        ...
        "ma probabilité de succès ne descend pas en dessous du plancher affiché — "
        "<i>Note : la VaR mesure un quantile de perte, pas le pire cas absolu (Expected Shortfall).</i> "
        "La métrique la plus risquée est <b>Neuro</b> (VaR la plus large), "
        "ce qui reflète la volatilité naturelle d'un profil neurodivergent. "
        "Cette même volatilité est aussi la source des pics de surperformance observés sur la série.",
        style="orange",
    )
    ...
    with col1:
        ...
        fig_hist.update_xaxes(tickformat=".1%", title="Rendement quotidien")
    ...
    with col2:
        ...
        fig_mc.update_xaxes(title="Jours")
        fig_mc.update_yaxes(tickformat=".2%", title="Rendement cumulé")
    ...
    fig_cone.update_yaxes(tickformat=".0%", title="Probabilité")
    fig_cone.update_xaxes(title="Date")
