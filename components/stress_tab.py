...
def render(df: pd.DataFrame):
    ...
    st.markdown(
        '<div class="terminal-header">STRESS TESTS',
        unsafe_allow_html=True,
    )

    info_box(
        "Explications :",
        "Les <b>Stress Tests</b> permettent de simuler des chocs extrêmes pour évaluer la résistance "
        "d'un portefeuille. Ici, ils répondent à la question : "
        "<i>\"Que se passe-t-il si tout va mal en même temps ?\"</i> "
        "et surtout : <b>combien de temps faut-il pour s'en remettre ?</b> "
    ...
    for scenario_name, scenario_info in SCENARIOS.items():
        ...
        fig_impact.update_yaxes(title="Probabilité (%)", range=[0, max(before_vals) * 1.3])
        ...
        fig_recovery.update_yaxes(tickformat=".0%", title="Probabilité")
        fig_recovery.update_xaxes(title="Jours après le choc")
    ...
    info_box(
        "Résultats :",
        "Même dans le pire scénario (crise de confiance à -40%), "
        "la métrique <b>Neuro</b> affiche le temps de recovery le plus court (30 jours dans cette simulation). "
        "Hypothèse : la neurodivergence favorise une capacité de rebond structurellement plus rapide. "
        "À valider empiriquement — mais c'est précisément la thèse que je défends. "
